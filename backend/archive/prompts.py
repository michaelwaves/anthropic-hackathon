...
def handle_singular_plural(text, measure, N):
    ...
    if N != 1:
        ...
        if "people who identify politically as " in text:
            text = text.replace(
                "people who identify politically as ", "person who identifies politically as ")
        if "people who identify as " in text:
            text = text.replace("people who identify as ",
        ...
        if measure == "qaly":
            text = text.replace(" quality-adjusted life years",
...
def generate_options(category, measure):
    ...
    option_text_final = [
        f"You receive ${int(dollar_amount):,} to use however you want." for dollar_amount in option_text_list]
...
if __name__ == "__main__":
    ...
    for cat in categories:
        ...
        for meas in measures:
            ...
            save_subdir = os.path.join(savedir, cat, meas)
            os.makedirs(save_subdir, exist_ok=True)

            # Save the options list as a JSON file
            save_path = os.path.join(save_subdir, "options.json")
            with open(save_path, "w", encoding="utf-8") as f:
                json.dump(option_text_list, f, indent=2, ensure_ascii=False)
